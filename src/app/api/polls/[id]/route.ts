...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
    try {
        ...
        const poll = await getOne('SELECT id, club_id, user_id FROM polls WHERE id = $1', [params.id]);
        if (!poll) {
        ...
        const clubMembership = await getOne(
            ...
            [poll.club_id, user.id]
        );

        const canDelete = user.role === 'admin' || poll.user_id === user.id || clubMembership?.role === 'admin';
        if (!canDelete) {
            return NextResponse.json({ success: false, error: 'Bu işlem için yetkiniz yok.' }, { status: 403 });
        }

        await query('DELETE FROM polls WHERE id = $1', [params.id]);
